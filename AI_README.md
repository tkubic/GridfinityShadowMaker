# AI_README — Project Summary for Assistants

Purpose
- Provide concise, focused context an AI assistant needs to contribute meaningfully to the project without prior history.
- Highlight architecture, data flows, key files, runtime commands, conventions, and high-priority tasks.
...
Runtime commands (developer)
- Full setup (Windows PowerShell):
  - `.\	ools\setup.ps1` — (or `.\	ests\scripts\setup.ps1` depending on location) — creates `.venv`, installs packages, runs `npm install`
...
Suggested next tasks (high value)
...
- Add CI step that runs lints, builds the frontend, and executes a smoke test against the backend in a containerized environment.
...
- Author: repository owner (local developer)
- Development preference: Windows + PowerShell, but the stack runs on Linux/macOS with small adjustments.

If you'd like, I can also:
- Add an `AI_README.short.md` summary for quick reference, or
- Generate an automated checklist of onboarding steps and tests as GitHub Issues or local TODOs.
